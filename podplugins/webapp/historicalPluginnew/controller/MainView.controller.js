  ...
  function(
    ...
    return PluginViewController.extend('rose.ext.podplugins.historicalPluginnew.controller.MainView', {
      ...
      onOrderValueHelpRequest: function() {
        ...
        if (!this.oOrderVHDia) {
          //Load the fragment
          this.oOrderVHDia = sap.ui.xmlfragment('rose.ext.podplugins.historicalPluginnew.view.fragments.OrderValueHelpRequest', this);
      ...
      onResourceValueHelpRequest: function() {
        ...
        if (!this.oResourceVHDia) {
          // Load the fragment
          this.oResourceVHDia = sap.ui.xmlfragment('rose.ext.podplugins.historicalPluginnew.view.fragments.ResourceValueHelpRequest', this);

          this.oResourceVHDia.getTableAsync().then(
            function(oTable) {
              // Add columns to the table
              oTable.addColumn(
      ...
      onMaterialValueHelpRequest: function() {
        var oView = this.getView(),
          oMatVhDialog = oView.byId('matVhDialog');
        ...
        Fragment.load({
          id: oView.getId(),
          name: 'rose.ext.podplugins.historicalPluginnew.view.fragments.MaterialValueHelpDialog',
          controller: this
        }).then(oDialog => {
          //Handle escape button press for the dialog
          oDialog.setEscapeHandler(oPromise => {
            //TODO: Handle dialog close - cleanup dialog contents
            // this.onCloseMaterialVhDia();
            oPromise.resolve();
          });
      ...
      onMaterialVhDiaCancel: function() {
        var oDialog = this.getView().byId('matVhDialog');
        if (oDialog) oDialog.close();
      },

      onMatVhSearchLiveChange: function(oEvent) {
        ...
        var oTable = this.getView().byId('matVhDiaTable');
        oTable.getBinding('items').filter(oFilter);
      },

      onMatVhDiaFilterBarChange: function(oEvent) {
        var sValue = oEvent.getParameter('newValue');
      ...
      onOperatorValueHelpRequest: function() {
        if (!this.oOperatorVHDia) {
          ...
          this.oOperatorVHDia = sap.ui.xmlfragment('rose.ext.podplugins.historicalPluginnew.view.fragments.OperatorValueHelpRequest', this);
          this.getView().addDependent(this.oOperatorVHDia);

          // Configure table columns in the dialog
          this.oOperatorVHDia.getTableAsync().then(
            function(oTable) {
              oTable.addColumn(
                new Column({
      ...
      onExportToExcel: function() {
        ...
        var aVisibleColumns = Object.entries(oColumnsConfig).filter(([key, isVisible]) => isVisible).map(([key]) => key);
